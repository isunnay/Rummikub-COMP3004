...
public class RummikubController implements Subject {
	...
	public void updateCurrentPlayer() {
		...
		String whosPlaying = "";
		if (aiOrNaw) {
			whosPlaying = "(AI"+allPlayers.get(whosTurn() - 1).getAiType()+")";
		} else {
			whosPlaying = "(Human)";
		}
		whosTurnLabel.setText(String.valueOf(whosTurn()-1) + whosPlaying);
	...
	private void handleDrop(MouseEvent event) {
		...
		if (tilePane.getChildren().contains(node)) {
			...
			int x = getSpotX(node);
			int y = getSpotY(node);
			if (x != -1 && y != -1 && board.getSpot(x, y).isSpotEmpty()) {
	...
	public void handleAI(int i) {
		for (int x = 0; x < this.allPlayers.get(i).getTurnTiles().size(); x++) {

			Tile tile = allPlayers.get(i).getTurnTiles().get(x);
			Spot spot = tile.getSpot();
			tile.setManaged(true);
			// GridPane.setConstraints(tile,spot.getSpotX(),spot.getSpotY());
			GridPane.setColumnIndex(tile, spot.getSpotX());
			GridPane.setRowIndex(tile, spot.getSpotY());
			tilePane.getChildren().remove(tile);
			gridPane.getChildren().addAll(tile);
...
//		System.out.println("Node LayoutX: " + node.getLayoutX());
			...
			for (int row = 0; row < 15; row++) {
				if (board.isSpotFilled(col, row)) {
					...
					if (!(tempArrayList.contains(board.getSpot(col, row)))) {
						...
						getNodeFromGridPane(gridPane, col, row)
		...
		for (int row = 0; row < 15; row++) {
			for (int col = 0; col < 15; col++) {
				if (board.isSpotFilled(row, col) && snapshot.isSpotFilled(row, col)
						&& board.getTileAtSpot(row, col).equals(snapshot.getTileAtSpot(row, col))) {
					System.out.println("old tiles in play at: " + row + "," + col);
					getNodeFromGridPane(gridPane, row, col)
	...
	public void drawTile() {
		int who = whosTurn() - 1;
		// allPlayers.get(who).setHasInitialMeldBeenPlayed(true);

		board.boardToString();
		if (allPlayers.get(who).isAI() == true || allPlayers.get(who).hasInitialMeldBeenPlayed() == true) {
			if (board.checkIfValidMelds() == false) {
				System.out.println("AIAIAIAIAIAIAIAIAIAIAIAIAAIAI");
				// Adding back to hand
				...
				// Adding back to old board location
					...
					for (int i = 0; i < nodesOnTurnGridToGrid.size(); i++) {
						...
						if (tile.justSwitched == false
								&& gridPane.getChildren().contains(nodesOnTurnGridToGrid.get(i))) {
	...
	public int getTurnScore() {
		...
		for (int i = 0; i < nodesOnTurnTileToGrid.size(); i++) {
			Tile t = (Tile) nodesOnTurnTileToGrid.get(i);
			System.out.println("This is what I played: " + t.tileToString());
	...
	public boolean onlyMeldsPlayed() {
		// This only works if everything you played is valid. so check if board is valid
		// before calling this.
		boolean onlyMeldsPlayed = true;

		for (int i = 0; i < board.getNumberOfMelds(); i++) {
			boolean meldTracker = true;
			int wholeMeld = 0;
			System.out.println(board.getMeld(i).meldToString());
			for (int j = 0; j < board.getMeld(i).getMeldSize(); j++) {
				System.out.println(board.getMeld(i).getTileInMeld(j).tileToString());
				if (!(nodesOnTurnTileToGrid.contains(board.getMeld(i).getTileInMeld(j)))) {
					meldTracker = false;
					wholeMeld++;
				}

				if (wholeMeld == board.getMeld(i).getMeldSize()) {
					meldTracker = true;
				}
			}

			if (!meldTracker) {
				onlyMeldsPlayed = false;
				break;
			}
		}

		if (nodesOnTurnTileToGrid.size() < 3) {
			onlyMeldsPlayed = false;
		}

		System.out.println("DID YOU ONLY PLAY MELDS? " + onlyMeldsPlayed);
		return onlyMeldsPlayed;
	}
	...
	public void determineStarter() {
		// TD = Tiles Drawn
		// MV = Max Value
		int p1TD = 0, p2TD = 0, p3TD = 0, p4TD = 0, p1MV = 0, p2MV = 0, p3MV = 0, p4MV = 0;
		...
		while (true) {
			...
			if (p1MV == 0) {
				allPlayers.get(0).getHand().getTile(p1TD).getValue();
				p1TD++;
			}
			if (p2MV == 0) {
				allPlayers.get(1).getHand().getTile(p2TD).getValue();
				p2TD++;
			}
			if (p3MV == 0) {
				allPlayers.get(2).getHand().getTile(p3TD).getValue();
				p3TD++;
			}
			if (p4MV == 0) {
				allPlayers.get(3).getHand().getTile(p4TD).getValue();
				p4TD++;
			}
			...
			if (p1MV == p2MV) {
				...
				/*
				 * System.out.println(allPlayers.get(0).getHand().getTile(p1TD).tileToString() +
				 * " - " + p1TD);
				 * System.out.println(allPlayers.get(1).getHand().getTile(p2TD).tileToString() +
				 * " - " + p2TD);
				 * System.out.println(allPlayers.get(2).getHand().getTile(p3TD).tileToString() +
				 * " - " + p3TD);
				 * System.out.println(allPlayers.get(3).getHand().getTile(p4TD).tileToString() +
				 * " - " + p4TD);
				 */
				p1TD++;
				p2TD++;
		...
		System.out.println("P" + checkHighestValue(p1MV, p2MV, p3MV, p4MV)
				+ " has the highest value and therefore will start the game.");
	...
	/*
	 * 
	 * public void playTurn(int i) { printAll(); // Play if human if
	 * (allPlayers.get(i).isAI() == false && allPlayers.get(i).myTurnStatus() ==
	 * true) { System.out.println("Player " + (i+1) + "'s Hand[" +
	 * allPlayers.get(i).getHand().size + "]: " +
	 * allPlayers.get(i).getHand().handToString()); try {
	 * allPlayers.get(i).play(reader, deck); } catch (InterruptedException e) {
	 * e.printStackTrace(); } } // Play if AI if (allPlayers.get(i).isAI() == true
	 * && allPlayers.get(i).myTurnStatus() == true) {
	 * allPlayers.get(i).play(reader);
	 * 
	 * if (allPlayers.get(i).hasTilesBeenPlayed() == false) { Tile t =
	 * allPlayers.get(i).getHand().dealTile(deck);
	 * System.out.println("Turn ended: Player " + (i+1) +
	 * " has decided to draw a tile."); System.out.println("Tile drawn: " +
	 * t.tileToString()); nextPlayersTurn(i); } } // Sets next players turn
	 * nextPlayersTurn(i); }
	 */
...
//			whosTurnLabel.setText("Player 4");
	...
	/*
	 * public void getWhosTurn() { if (allPlayers.get(0).myTurnStatus() == true) {
	 * whosTurnLabel.setText("Player 1"); } else if
	 * (allPlayers.get(1).myTurnStatus() == true) {
	 * whosTurnLabel.setText("Player 2"); } else if
	 * (allPlayers.get(2).myTurnStatus() == true) {
	 * whosTurnLabel.setText("Player 3"); } else if
	 * (allPlayers.get(3).myTurnStatus() == true) {
	 * whosTurnLabel.setText("Player 4"); }
	 * 
	 * }
	 */
